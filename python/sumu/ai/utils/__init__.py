...
# NOTE: `torch` is intentionally not imported here. The `type ... = torch.Tensor` aliases below
# are PEP 695 lazy type aliases (their value is only computed on first access to `.__value__`,
# which nothing in this codebase does) - importing torch eagerly at this module's top would force
# the CUDA/cuDNN runtime to load merely from `from sumu.ai.utils import VideoMetadata`, which sits on
# the GUI's startup import chain (see lada/gui/frame_restorer_provider.py).
