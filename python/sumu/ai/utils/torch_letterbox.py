...
class PyTorchLetterBox:
    def __init__(self, imgsz: int | tuple[int, int], original_shape: tuple[int, int], stride: int = 32) -> None:
        ...
        # uint8 [0,255] range: input frames are uint8 (matching the CPU LetterBox path, which pads
        # 114 and normalises /255 later in inference_and_postprocess). Padding 114/255 here would
        # round to black on a uint8 tensor.
