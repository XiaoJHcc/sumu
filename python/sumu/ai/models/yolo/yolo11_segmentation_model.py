...
from sumu.ai.utils.torch_letterbox import PyTorchLetterBox
from sumu.ai.utils.ultralytics_utils import UltralyticsResults
...
class Yolo11SegmentationModel:
    ...
    def _preprocess_gpu(self, imgs: list[ImageTensor]) -> torch.Tensor:
        # imgs are HWC uint8 (BGR) GPU tensors; PyTorchLetterBox expects BCHW. Stack then permute
        # BHWC -> BCHW (matches the CPU path's transpose at _preprocess_cpu). div/255 happens later.
        batch = torch.stack(imgs, dim=0).permute(0, 3, 1, 2)
    ...
    def preprocess(self, imgs: list[ImageTensor]) -> list[torch.Tensor]:
        ...
        if is_cpu_input:
            ...
            # self.letterbox starts as an ultralytics LetterBox (CPU path); the GPU path lazily
            # builds a PyTorchLetterBox keyed on the input shape. Rebuild if it isn't a
            # PyTorchLetterBox yet (first GPU frame) or the frame resolution changed.
            if not isinstance(self.letterbox, PyTorchLetterBox) or imgs[0].shape[:2] != self.letterbox.original_shape:
