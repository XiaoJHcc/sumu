...
native/              ★ C++ 原生内核（present + decode + interop + ready-map）
  CMakeLists.txt
  src/               present loop、D3D11 swapchain、decode、CUDA interop、pybind11 绑定
  shaders/           NV12→RGB 等 HLSL
python/sumu/         Python 侧 AI 编排（Phase 4 照搬 lada 核心）
spikes/              ★ 逐个 spike 的独立验证代码（spike0/1/2）
scripts/             analyze_present.py（present 上屏间隔分析，PresentMon/ns 双格式）等
docs/                spike 结果记录、设计文档
...
1. **4K60 HEVC 硬解达正经播放器水平**（Spike 0，硬门）。
2. **架构预留 AI 画面插入能力**（Spike 1，零拷贝 CUDA↔D3D11）。
3. **AI 模型能在此架构运行对接**（Phase 4，照搬 lada 核心）。
