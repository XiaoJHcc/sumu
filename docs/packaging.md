...
- **权重源目录解析顺序**（其他机器/团队成员构建时不用改脚本）：`-WeightsSrc` 显式参数 → `$env:SUMU_WEIGHTS_SRC` 环境变量（`setx SUMU_WEIGHTS_SRC "C:\path\to\model_weights"` 设一次，跨会话生效）→ `$env:LADA_MODEL_WEIGHTS_DIR`（legacy 兼容）→ 仓库本地 `model_weights/`（自包含的文档化位置，运行期解析也默认它）。四者均未命中时脚本直接报错退出——不假定任何外部仓库布局。缺文件时报错会指出具体缺哪个文件。
