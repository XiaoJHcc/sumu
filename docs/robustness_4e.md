...
**再更新（后续 commit，播放行为变更）：播放器行为已改为"播完停在最后一帧、自动暂停，
不再循环回绕"（切换视频/重新播放均从头开始，不存在"回绕到 frame 0"）。因此本节③记录的
"EOF 循环回绕"路径在当前日常播放中不会被触发，`decoder.cpp` 里的 `AVERROR_EOF` 修法
仍然保留且仍是正确的防御（对应"播完停在最后一帧"而非冻屏），但 `scheduler.py` 侧
"循环回绕后 frame_num 单调爬升、eof 取模" 相关的分析、以及下方遗留的"多次循环边界累积态"
验证缺口，均已随此行为变更失去日常触发路径，仅供架构历史参考，不再是待办验证项。**
...
建议修法实现并实测通过。产品策略随后改为**播完停最后一帧、不循环**，回绕路径
不再是日常触发面。

### 历史局限（不再当待办）

本节原「仍存局限」多为单次压测方法论边界或已废弃的 loop 路径。**第一阶段已收口**
（见 CLAUDE.md）：4K 命中率≈0 是 best-effort 产品现实而非架构债；多次回绕/回绕+seek
混合在「播完即停」策略下无日常触发路径；不再驱动后续工作。
